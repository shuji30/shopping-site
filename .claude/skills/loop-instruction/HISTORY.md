...
## loop 80 — ループ報告の末尾に完了時刻を出す規約を追加（2026-08-22）

### 経緯
- ユーザーからの依頼:「ループの報告に完了した時間を最後に表示するようスキルに追加して」。
- 20分ループを回していると、チャットの流れだけでは「どのループがいつ終わったか」が
  分からない。コミット日時を見に行けば分かるが、報告そのものに時刻が無いのは不便。

### やったこと
- `SKILL.md`「ループの実行手順」に **手順8「報告」** を追加。
- 新しい節「ループ完了時刻の表示」を追加し、次を規約化した:
  - 報告は**必ず完了時刻の1行で終える**
  - 時刻は推測せず、報告を書く直前に `TZ=Asia/Tokyo date "+%Y-%m-%d %H:%M:%S JST"`
    で取得する（この環境のシステム時計は UTC。ユーザーは日本時間で読むため）
  - 書式は `完了時刻: 2026-08-22 10:21:45 JST（loop 80）`
  - コミット/push まで終えたあとの時刻を取る（着手時刻ではない）
  - 中断・失敗で終わる場合も同じ書式で残す

### 気づき
- モデルは現在時刻を持っていないので、「時刻を書く」規約は必ず**取得コマンドとセット**で
  書かないと、それらしい嘘の時刻を書いてしまう。書式だけ決めても不十分。

---

## loop 79 — 商品マスタ管理の土台（検証＋サーバーアクション）（2026-08-21）

### 経緯
- loop 77 で起票した「商品マスタの管理」に着手。フィールドが11個あり
  配列項目の変換も要るので、**土台（検証・サーバーアクション）とUIを分ける**。
  今回は土台まで。画面はまだ無いので、この時点で挙動の変化はない。
- なお loop 76〜78 は ROADMAP への起票と記録の更新のみで、HISTORY に個別の
  エントリは作っていない（内容はコミットログと ROADMAP を参照）。

### やったこと
- `lib/kimono-validation.ts`（新規・純粋関数）: カテゴリ（loop 72）と同じ方針。
  - `validateKimono(input, mode)` … `mode: "create"` のときだけ識別子を検証する
  - `parseList` … 「S, M, L」「赤，金、銀・白 黒」などを配列に。**全角カンマ・読点・
    中黒・空白**を区切りとして扱い、空要素と重複を落として順序は保つ
  - `formatList` … 配列をフォームへ戻す（`parseList` と往復できる）
  - `parsePositiveInt` … 桁区切りカンマを許し、0・負数・小数・非数値は null
  - 料金と日数に上限（100万円 / 60日）。桁の打ち間違いを弾くため
- `lib/actions/admin-kimono.ts`（新規）: `createKimono` / `updateKimono` / `deleteKimono`。
  - sizes/colors/images は配列 → JSON文字列に変換して保存（SQLiteに配列型が無いため）
  - **カテゴリはマスタに存在するものだけ許す**（存在しないと一覧から辿れなくなる）
  - 識別子は登録後に変更不可（商品URL と `ReservationItem.kimonoId` が壊れる）
  - **予約明細から参照されている商品は削除不可**。掲載を止めるだけなら「在庫なし」に
  - 削除時はレビューも一緒に消す（残すと孤児になる）
- `lib/kimono-repository.ts`: `countReservationItemsForKimono` を追加（削除可否の判定）。
- `tests/kimono-validation.test.ts`: 23件追加（132→155件）。

### 気づき
- カテゴリ削除は「商品が紐づいていたら拒否」だったが、商品削除は**予約明細**が相手。
  予約は過去の事実なので、商品を消しても履歴の商品名・価格は `ReservationItem` に
  コピー済みで壊れない。それでも削除を拒否するのは、管理画面から辿れなくなるのと、
  「消した商品の予約」という状態が運用上わかりにくいため。
- レビューは商品に完全従属なので一緒に削除する。ここはカテゴリと非対称。

### 結果
- ESLint 0・`tsc --noEmit` エラー0・vitest 155件パス・`next build` 成功。
- **画面はまだ無いので e2e は次ループ**。この時点では挙動が変わらないことを
  build とテストで確認するに留めた。

### 次への申し送り
- 次は `/admin/kimonos` の一覧・新規・編集フォーム。カテゴリは
  `getCategories()` で取ってセレクトボックスにする。
- 配列項目の入力欄には「カンマ区切り」の説明を必ず添えること。
