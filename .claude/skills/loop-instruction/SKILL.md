...
8. **報告**: ユーザーへの報告の**最後に、そのループが完了した時刻を必ず表示する**
   （詳細は下記「ループ完了時刻の表示」）。
...
ループの報告（ユーザーへ返すメッセージ）は、**必ず完了時刻の1行で終える**。
どのループがいつ終わったのかを、後からチャットの流れだけで追えるようにするため。

- 時刻は**推測で書かない**。モデルは現在時刻を持っていないので、報告を書く直前に
  シェルで取得する:
  ```bash
  TZ=Asia/Tokyo date "+%Y-%m-%d %H:%M:%S JST"
  ```
  （この環境のシステム時計は UTC。ユーザーは日本時間で読むため `TZ=Asia/Tokyo` を付ける）
- 書式は報告の末尾に次の1行を置く:
  ```
  完了時刻: 2026-08-22 10:21:45 JST（loop 80）
  ```
- コミットや push まで含めて終わったあとに取得すること（着手時刻ではなく**完了時刻**）。
- 途中で中断・失敗して終わる場合も、その時点の時刻を同じ書式で残す
  （「どこまでやっていつ止まったか」が分かるようにするため）。
...
- **本番反映は Pull Request 経由を既定とする**（loop 70）。`develop` → `master` の
  PR を作り、CI が通ったことを確認してからマージする。何を本番に出したか・
  動作確認したか・反映時の注意が記録に残り、切り戻しの単位も明確になるため。
  PR本文は `.github/pull_request_template.md` の枠を埋める（とくに
  「本番反映時の注意」「デプロイ後の確認」は、マージする人が読む前提で必ず書く）。
  急ぎ等でユーザーが直マージを指示した場合はそれに従ってよい。
  ブランチ保護（CI必須化）は未設定＝CIが赤くてもマージできてしまうので、
  **マージ前にCIの結果を目視で確認すること**（設定手順は `web/docs/CI-CD.md`）。
